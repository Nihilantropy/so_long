...
decode_and_replace() {
    ...
    while IFS= read -r line; do
        if [[ "$line" == "# define"* && ! "$line" =~ "ifndef|endif" ]]; then
            ...
            decoded_string=$(./roba_inutile_di_lavoro/encriptor.out "$string")
            decoded_variable=$(./roba_inutile_di_lavoro/encriptor.out "$variable")
...
echo "Files successfully encrypted"
